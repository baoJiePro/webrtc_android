 ...
 */
internal class WanApiResponseConverter<T>(private val type: Type, private val gson: Gson, private val adapter: TypeAdapter<T> ) : Converter<ResponseBody, Any?> {
    ...
    override fun convert(value: ResponseBody): Any? {
        ...
        return if (jsonElement.isJsonObject) {
            with(jsonElement.asJsonObject) {
                ...
                get(WanApiKeyConstant.KEY_CODE)?.apply {
                    code = this.asString
                }
                if (code == null) {
                    code = "-1"
                ...
                var msg: String?
                msg = get(WanApiKeyConstant.KEY_MSG)?.asString
                if (msg == null) {
                    msg = ""
                }

                if (code == ApiResultCode.SUCCESS_CODE_0) {
                    val contentJsonElement = get(WanApiKeyConstant.KEY_DATA)
                    when {
